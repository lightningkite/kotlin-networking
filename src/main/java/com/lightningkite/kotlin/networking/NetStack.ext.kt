...
fun NetStack.response(request: NetRequest) = stream(request).response()

fun NetStack.async(request: NetRequest, onResponse: (NetResponse) -> Unit) {
    doAsync(
            action = { stream(request).response() },
            uiThread = { onResponse(it) }
    )
}
